...
@onready var game_flow: GameLogicState = $StrategicSystem
@onready var world_map: MapSystem = $WorldMap
@onready var ship_battle: LegacyShipBattle = $ShipBattle
@onready var main_camera: Camera2D = $Camera2D
@onready var tactical_combat: TacticalCombatSystem = $TacticalCombatSystem
@onready var ship_combat_v2: ShipCombatManager = $ShipCombatV2

var combat_renderer: LegacyCombatRendererV2
...
var sidebar_wind_compass: Control
var sidebar_ship_status_label: Label
...
var last_ship_battle_encounter_context: Dictionary = {}
...
var player_ship_hull_current: int = 0
...
var port_trade_repair_label: Label
var port_trade_repair_button: Button
...
var ship_move_selected_heading_idx: int = -1
const SHIP_HEADING_LABELS: PackedStringArray = ["N", "NE", "E", "SE", "S", "SW", "W", "NW"]
var ship_combat_selected_action: int = -1
...
## -1 = none, 0 = port range, 1 = starboard range
var ship_combat_range_highlight: int = -1
var port_menu_overlay: ColorRect
...
var boarding_initiative_order: Array[CombatUnit] = []
...
var boarding_initial_player_boarders: int = 0
var boarding_initial_enemy_boarders: int = 0
...
const SIDEBAR_WIND_COMPASS_MIN_HEIGHT := 92.0
const SIDEBAR_WIND_COMPASS_BOTTOM_GAP := 8.0
...
const SHIP_ACTION_RANGE_PORT := 1
const SHIP_ACTION_RANGE_STARBOARD := 2
const SHIP_ACTION_FIRE_CANNONS := 3
const SHIP_ACTION_BOARD := 4
const SHIP_ACTION_DISENGAGE := 5
const SHIP_ACTION_END_TURN := 6
const SHIP_COMBAT_SAIL_ROW_Y := 14.0
const SHIP_COMBAT_CANNON_RANGE_LABEL_Y := 78.0
const SHIP_COMBAT_RANGE_ROW_Y := 84.0
const SHIP_COMBAT_ACTION_ROW_Y := 130.0
const SHIP_COMBAT_FRAMING_CELL_PAD := 8
const SHIP_COMBAT_MIN_ZOOM := 0.7
const SHIP_COMBAT_MAX_ZOOM := 8.0
const BOARDING_ACTION_MOVE := 0
const BOARDING_ACTION_RANGED := 1
const BOARDING_ACTION_MELEE := 2
const BOARDING_ACTION_END_TURN := 3
const CREW_CARD_LABELS := "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
const SUPPLY_UNIT_COST := 6
const HULL_REPAIR_COST_PER_POINT := 28
const CREW_ROSTER_HARD_MAX := 120
...
class SidebarWindCompassControl extends Control:
	var host: Node2D
	var _last_compass_key: String = ""

	func _ready() -> void:
		set_process(true)

	func _process(_delta: float) -> void:
		if host == null or not host.has_method("_sidebar_compass_invalidate_key"):
			return
		var key: String = host._sidebar_compass_invalidate_key()
		if key != _last_compass_key:
			_last_compass_key = key
			queue_redraw()

	func _draw() -> void:
		if host == null or not host.has_method("_paint_sidebar_wind_compass"):
			return
		host._paint_sidebar_wind_compass(self)


func _ready() -> void:
	randomize()
	combat_renderer = LegacyCombatRendererV2.new(self)
	_reset_career_state()
	...
	if ship_combat_v2:
		ship_combat_v2.combat_finished.connect(_on_ship_combat_v2_finished)
...
func _process(delta: float) -> void:
	...
	if viewport_size != last_viewport_size:
		...
		_layout_worldmap_ui()
		_layout_tactical_ui()
	if game_flow.current_mode == GameLogicState.Mode.SHIP_COMBAT and ship_battle.phase == LegacyShipBattle.Phase.MOVE_ANIM:
	...
	if game_flow.current_mode == GameLogicState.Mode.SHIP_COMBAT or (
		game_flow.current_mode == GameLogicState.Mode.TACTICAL_COMBAT and game_flow.tactical_type == GameLogicState.TacticalType.BOARDING
	):
		queue_redraw()
...
func _draw() -> void:
	if game_flow.current_mode == GameLogicState.Mode.SHIP_COMBAT:
		_sync_ship_move_selection()
		combat_renderer.draw_ship_combat(self, get_viewport_rect().size)
	elif game_flow.current_mode == GameLogicState.Mode.TACTICAL_COMBAT and game_flow.tactical_type == GameLogicState.TacticalType.BOARDING:
		combat_renderer.draw_boarding_combat(self)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if game_flow.current_mode == GameLogicState.Mode.WORLD_MAP and world_map.handle_input(event):
		return

	if event is InputEventMouseMotion and game_flow.current_mode == GameLogicState.Mode.TACTICAL_COMBAT:
		var local_pos := grid.to_local(get_global_mouse_position())
		grid.set_hovered_cell(grid.local_to_cell(local_pos))

	if game_flow.current_mode == GameLogicState.Mode.SHIP_COMBAT and event is InputEventMouseMotion and ship_combat_is_panning:
	...
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		match game_flow.current_mode:
			GameLogicState.Mode.WORLD_MAP:
				_handle_world_map_click(get_global_mouse_position())
			GameLogicState.Mode.SHIP_COMBAT:
				if ship_battle.phase == LegacyShipBattle.Phase.PLANNING:
					_handle_ship_combat_click(get_global_mouse_position())
			GameLogicState.Mode.TACTICAL_COMBAT:
				_handle_tactical_click(get_global_mouse_position())

	if game_flow.current_mode == GameLogicState.Mode.SHIP_COMBAT and event is InputEventMouseButton:
	...
	if game_flow.current_mode == GameLogicState.Mode.SHIP_COMBAT and ship_battle.phase == LegacyShipBattle.Phase.PLANNING:
	...
	if event.is_action_pressed("ui_accept"):
		match game_flow.current_mode:
			GameLogicState.Mode.SHIP_COMBAT:
				if ship_battle.phase == LegacyShipBattle.Phase.PLANNING:
					if ship_battle.is_player_movement_plotted() and ship_battle.can_player_execute_move_leg():
						ship_battle.player_execute_move_leg()
					elif (
						ship_move_selected_cell.x >= 0
						and ship_move_selected_heading_idx >= 0
						and ship_battle.can_player_move_to_option(
							ship_move_selected_cell, ship_move_selected_heading_idx
						)
					):
						_confirm_ship_move_selection()
			GameLogicState.Mode.TACTICAL_COMBAT:
				if game_flow.tactical_type != GameLogicState.TacticalType.BOARDING:
					_end_turn()

	if event is InputEventKey and event.pressed and not event.echo:
		var key_event: InputEventKey = event
		if game_flow.current_mode == GameLogicState.Mode.SHIP_COMBAT and ship_battle.phase == LegacyShipBattle.Phase.PLANNING and key_event.keycode == KEY_F:
			if ship_combat_range_highlight >= 0:
				ship_battle.player_fire_battery(ship_combat_range_highlight)
			else:
				game_flow.post_message("Highlight Range Port or Starboard, then press F or Fire Cannons.")
		elif game_flow.current_mode == GameLogicState.Mode.SHIP_COMBAT and ship_battle.phase == LegacyShipBattle.Phase.PLANNING and key_event.keycode == KEY_E:
			ship_battle.player_end_turn()
		elif game_flow.current_mode == GameLogicState.Mode.SHIP_COMBAT and ship_battle.phase == LegacyShipBattle.Phase.PLANNING and key_event.keycode == KEY_BRACKETLEFT:
			ship_battle.cycle_player_sail(-1)
			_sync_ship_move_selection()
		elif game_flow.current_mode == GameLogicState.Mode.SHIP_COMBAT and ship_battle.phase == LegacyShipBattle.Phase.PLANNING and key_event.keycode == KEY_BRACKETRIGHT:
			ship_battle.cycle_player_sail(1)
			_sync_ship_move_selection()
		elif game_flow.current_mode == GameLogicState.Mode.TACTICAL_COMBAT and game_flow.tactical_type == GameLogicState.TacticalType.BOARDING and key_event.keycode == KEY_E:
			_end_boarding_actor_turn()
		elif game_flow.current_mode == GameLogicState.Mode.SHIP_COMBAT and ship_battle.phase == LegacyShipBattle.Phase.PLANNING and key_event.keycode == KEY_X:
			if ship_battle.can_player_disengage():
				ship_battle.player_disengage()

	if event.is_action_pressed("ui_select") and game_flow.current_mode == GameLogicState.Mode.TACTICAL_COMBAT:
		game_flow.post_message("Manual resolve removed. Win by combat/objectives.")

	if event.is_action_pressed("ui_cancel"):
		if game_flow.current_mode == GameLogicState.Mode.TACTICAL_COMBAT:
			_clear_selection()
...
func _setup_tactical_units() -> void:
	...
	if game_flow.tactical_type == GameLogicState.TacticalType.BOARDING:
		...
		if current_boarding_is_player_attacking:
			_spawn_units_from_cells(CombatTurnManager.Team.PLAYER, boarding_attacker_spawn_cells)
			_spawn_units_from_cells(CombatTurnManager.Team.ENEMY, boarding_defender_spawn_cells)
		else:
			_spawn_units_from_cells(CombatTurnManager.Team.PLAYER, boarding_defender_spawn_cells)
			_spawn_units_from_cells(CombatTurnManager.Team.ENEMY, boarding_attacker_spawn_cells)
	else:
		_clear_special_tactical_layout()
		_create_unit(CombatTurnManager.Team.PLAYER, Vector2i(2, 2))
		_create_unit(CombatTurnManager.Team.PLAYER, Vector2i(3, 4))
		_create_unit(CombatTurnManager.Team.ENEMY, Vector2i(10, 3))
		_create_unit(CombatTurnManager.Team.ENEMY, Vector2i(9, 6))
	_assign_boarding_unit_labels()
	_refresh_boarding_unit_visuals()
	_layout_tactical_ui()

func _create_unit(team: CombatTurnManager.Team, cell: Vector2i, hp: int = -1, max_hp: int = -1) -> CombatUnit:
...
func _spawn_units_from_cells(team: CombatTurnManager.Team, cells: Array[Vector2i]) -> void:
	for cell in cells:
...
func _handle_tactical_click(world_pos: Vector2) -> void:
	if game_flow.tactical_type == GameLogicState.TacticalType.BOARDING:
		...
		if panel_rect.has_point(world_pos):
			if _boarding_action_button_rect(panel_rect, BOARDING_ACTION_MOVE).has_point(world_pos):
				...
				var actor: CombatUnit = _current_boarding_actor()
				if actor != null and actor.team == CombatTurnManager.Team.PLAYER:
					_select_unit(actor)
			...
			elif _boarding_end_turn_button_rect(panel_rect).has_point(world_pos):
				_end_boarding_actor_turn()
			return

	var active_actor: CombatUnit = _current_boarding_actor() if game_flow.tactical_type == GameLogicState.TacticalType.BOARDING else null
	if game_flow.tactical_type == GameLogicState.TacticalType.BOARDING and (active_actor == null or active_actor.team != CombatTurnManager.Team.PLAYER):
		return
	...
	if selected_unit != null and clicked_unit != null and clicked_unit.team != selected_unit.team:
		if game_flow.tactical_type == GameLogicState.TacticalType.BOARDING and boarding_selected_action == BOARDING_ACTION_RANGED:
...
func _select_unit(unit: CombatUnit) -> void:
	if game_flow.tactical_type != GameLogicState.TacticalType.BOARDING and unit.has_acted:
		return
	if game_flow.tactical_type == GameLogicState.TacticalType.BOARDING and unit != _current_boarding_actor():
		return
	...
	if game_flow.tactical_type == GameLogicState.TacticalType.BOARDING and (boarding_selected_action != BOARDING_ACTION_MOVE or boarding_actor_moved):
...
func _on_turn_started(team: int) -> void:
	...
	if team == CombatTurnManager.Team.ENEMY:
		_take_enemy_tactical_turn()

func _take_enemy_tactical_turn() -> void:
	for unit in units:
		if unit.team != CombatTurnManager.Team.ENEMY:
			continue
		if unit.has_acted:
			continue
...
func _get_unit_at(cell: Vector2i) -> CombatUnit:
	for unit in units:
		if unit.cell == cell:
			return unit
	return null

func _blocked_cells_for(active_unit: CombatUnit) -> Dictionary:
	var blocked := {}
	if game_flow.tactical_type == GameLogicState.TacticalType.BOARDING:
...
func _unit_matches_current_team(unit: CombatUnit) -> bool:
	if game_flow.tactical_type == GameLogicState.TacticalType.BOARDING:
		return unit == _current_boarding_actor()
	if turn_manager.current_team == CombatTurnManager.Team.PLAYER:
		return unit.team == CombatTurnManager.Team.PLAYER
	return unit.team == CombatTurnManager.Team.ENEMY

func _set_mode(mode: GameLogicState.Mode) -> void:
...
func _on_mode_changed(new_mode: int) -> void:
	...
	# Handle ShipCombatV2 activation
	if ship_combat_v2:
		ship_combat_v2.set_active(new_mode == GameLogicState.Mode.SHIP_COMBAT_V2)

	# The world/gameplay HUD would otherwise render on top of the V2 naval scene,
	# which manages its own camera and UI.
	var hide_world_hud: bool = new_mode == GameLogicState.Mode.SHIP_COMBAT_V2
	if left_sidebar_layer != null:
		left_sidebar_layer.visible = not hide_world_hud
	if captain_log_layer != null:
		captain_log_layer.visible = not hide_world_hud
	if time_controls_layer != null:
		time_controls_layer.visible = not hide_world_hud

	var tactical_visible: bool = new_mode == GameLogicState.Mode.TACTICAL_COMBAT
	var boarding_tactical: bool = tactical_visible and game_flow.tactical_type == GameLogicState.TacticalType.BOARDING
	grid.visible = tactical_visible and not boarding_tactical
	units_root.visible = tactical_visible and not boarding_tactical
	world_map.visible = new_mode == GameLogicState.Mode.WORLD_MAP
	if port_menu_layer != null:
		port_menu_layer.visible = new_mode == GameLogicState.Mode.WORLD_MAP and port_menu_active
	if main_map_info_layer != null and new_mode != GameLogicState.Mode.WORLD_MAP:
		main_map_info_layer.visible = false
	if new_mode == GameLogicState.Mode.SHIP_COMBAT:
		ship_combat_selected_action = -1
		ship_combat_range_highlight = -1
		ship_combat_zoom = 2.5
	...
	if world_map.visible:
		call_deferred("_layout_worldmap_ui")
	if new_mode == GameLogicState.Mode.WORLD_MAP:
		_refresh_sidebar_ship_status()
...
func _on_boarding_started(attacker_is_player: bool) -> void:
	...
	game_flow.set_tactical_type(GameLogicState.TacticalType.BOARDING)
	boarding_active = true
	boarding_defender_hold_progress = 0
	_setup_tactical_units()
	_set_mode(GameLogicState.Mode.TACTICAL_COMBAT)
...
func _on_ship_battle_finished(player_won: bool, disengaged: bool = false) -> void:
	_sync_career_from_ship_battle()
	last_ship_battle_encounter_context.clear()
	...
	if disengaged:
		...
		_set_mode(GameLogicState.Mode.WORLD_MAP)
		return
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and not event.echo and event.keycode == KEY_T:
		if game_flow.current_mode == GameLogicState.Mode.WORLD_MAP and can_start_town_assault:
			_start_town_assault_demo()
		elif game_flow.current_mode == GameLogicState.Mode.WORLD_MAP:
...
func _start_town_assault_demo() -> void:
	game_flow.set_tactical_type(GameLogicState.TacticalType.TOWN_ASSAULT)
...
func _can_melee_attack(attacker: CombatUnit, defender: CombatUnit) -> bool:
...
func _perform_melee_attack(attacker: CombatUnit, defender: CombatUnit, silent: bool = false) -> void:
...
func _perform_ranged_attack(attacker: CombatUnit, defender: CombatUnit, silent: bool = false) -> void:
	if attacker == null or defender == null:
		return
	if game_flow.tactical_type == GameLogicState.TacticalType.BOARDING:
...
func _alive_count(team: CombatTurnManager.Team) -> int:
	var count := 0
	for unit in units:
		if unit.team == team:
			count += 1
...
func _start_boarding_initiative() -> void:
	...
	var players: Array[CombatUnit] = []
	var enemies: Array[CombatUnit] = []
	for unit in units:
		if unit != null and is_instance_valid(unit):
			unit.set_acted(false)
			if unit.team == CombatTurnManager.Team.PLAYER:
				players.append(unit)
			else:
				enemies.append(unit)
	players.shuffle()
	...
	boarding_round = 1
	boarding_selected_action = BOARDING_ACTION_MOVE
	boarding_initial_player_boarders = _alive_count(CombatTurnManager.Team.PLAYER)
	boarding_initial_enemy_boarders = _alive_count(CombatTurnManager.Team.ENEMY)
	_begin_boarding_actor_turn()

func _current_boarding_actor() -> CombatUnit:
	if game_flow.tactical_type != GameLogicState.TacticalType.BOARDING:
		return null
	...
	if boarding_initiative_index < 0 or boarding_initiative_index >= boarding_initiative_order.size():
		return null
	var actor: CombatUnit = boarding_initiative_order[boarding_initiative_index]
...
func _begin_boarding_actor_turn() -> void:
	...
	if actor.team == CombatTurnManager.Team.PLAYER:
		_select_unit(actor)
	else:
...
func _advance_boarding_turn() -> void:
	...
	if boarding_initiative_index >= boarding_initiative_order.size():
		...
		_update_boarding_defender_hold_after_round()
	_begin_boarding_actor_turn()

func _end_boarding_actor_turn() -> void:
	if game_flow.tactical_type != GameLogicState.TacticalType.BOARDING:
		return
	var actor: CombatUnit = _current_boarding_actor()
	if actor == null or actor.team != CombatTurnManager.Team.PLAYER:
		return
	boarding_actor_moved = true
	boarding_actor_attacked = true
	_clear_selection()
	game_flow.post_message("%s ends turn." % _unit_label(actor))
	_advance_boarding_turn()

func _prune_boarding_initiative() -> void:
	var actor_before: CombatUnit = _current_boarding_actor()
	var pruned: Array[CombatUnit] = []
...
func _take_enemy_boarding_action(unit: CombatUnit) -> void:
	if unit == null or unit.team != CombatTurnManager.Team.ENEMY:
		return
	boarding_actor_moved = false
	boarding_actor_attacked = false
	var adjacent_target: CombatUnit = _get_adjacent_enemy(unit)
	if adjacent_target != null:
		_perform_melee_attack(unit, adjacent_target, true)
		return
	var ranged_target: CombatUnit = _best_enemy_ranged_target(unit)
...
func _best_enemy_ranged_target(unit: CombatUnit) -> CombatUnit:
...
func _check_tactical_outcome() -> void:
	if game_flow.tactical_type == GameLogicState.TacticalType.BOARDING and boarding_active:
		if _has_attacker_on_objective():
			_finish_boarding_tactical(true)
			return
		if boarding_defender_hold_progress >= boarding_defender_hold_turns_required:
			_finish_boarding_tactical(false)
			return
	var player_alive := _alive_count(CombatTurnManager.Team.PLAYER)
	var enemy_alive := _alive_count(CombatTurnManager.Team.ENEMY)
	if player_alive <= 0:
		if boarding_active:
			_finish_boarding_tactical(not current_boarding_is_player_attacking)
		else:
			_handle_tactical_finished(false)
		return
	if enemy_alive <= 0:
		if boarding_active:
			_finish_boarding_tactical(current_boarding_is_player_attacking)
		else:
			_handle_tactical_finished(true)

func _finish_boarding_tactical(attackers_won: bool) -> void:
	boarding_active = false
	_clear_selection()
	var player_casualties: int = _boarding_crew_casualties_for_team(CombatTurnManager.Team.PLAYER)
	var enemy_casualties: int = _boarding_crew_casualties_for_team(CombatTurnManager.Team.ENEMY)
	ship_battle.apply_boarding_crew_casualties(player_casualties, enemy_casualties)
	_sync_career_from_ship_battle()
	ship_battle.resolve_boarding_result(current_boarding_is_player_attacking, attackers_won)
	_set_mode(GameLogicState.Mode.SHIP_COMBAT)

func _boarding_crew_casualties_for_team(team: CombatTurnManager.Team) -> int:
	var initial: int = (
		boarding_initial_player_boarders if team == CombatTurnManager.Team.PLAYER else boarding_initial_enemy_boarders
	)
	if initial <= 0:
		return 0
	var lost_units: int = maxi(0, initial - _alive_count(team))
	if lost_units <= 0:
		return 0
	var crew_start: int = (
		ship_battle.player_crew_battle_start if team == CombatTurnManager.Team.PLAYER else ship_battle.enemy_crew_battle_start
	)
	var per_boarder: int = maxi(1, int(round(float(crew_start) / float(initial))))
	return lost_units * per_boarder
...
func _update_boarding_defender_hold_after_round() -> void:
	if not boarding_active or game_flow.tactical_type != GameLogicState.TacticalType.BOARDING:
		return
	...
	game_flow.post_message(
		"Defenders held %d/%d rounds." % [boarding_defender_hold_progress, boarding_defender_hold_turns_required]
	)
	_check_tactical_outcome()

func _attackers_team_has_objective_control() -> bool:
	var attackers_team := CombatTurnManager.Team.PLAYER if current_boarding_is_player_attacking else CombatTurnManager.Team.ENEMY
	for unit in units:
...
func _get_adjacent_enemy(unit: CombatUnit) -> CombatUnit:
...
func _best_enemy_move_cell(unit: CombatUnit) -> Vector2i:
	var blocked := _blocked_cells_for(unit)
	...
	for ally in units:
		if ally.team == CombatTurnManager.Team.PLAYER:
...
func _first_los_blocker(from_cell: Vector2i, to_cell: Vector2i) -> Vector2i:
	...
	for cell in line_cells:
		if game_flow.tactical_type == GameLogicState.TacticalType.BOARDING and not grid.is_cell_on_deck(cell):
			return cell
		if boarding_obstacle_cells.has(cell):
			return cell
		var blocker: CombatUnit = _get_unit_at(cell)
...
func _cover_bonus_for(cell: Vector2i) -> int:
	var bonus: int = 0
	if game_flow.tactical_type == GameLogicState.TacticalType.BOARDING and boarding_chokepoint_cells.has(cell):
		bonus += 2
	...
	for offset in orthogonal:
		if boarding_obstacle_cells.has(cell + offset):
			...
			break
	if (
		game_flow.tactical_type == GameLogicState.TacticalType.BOARDING
		and bonus < 2
		and not boarding_chokepoint_cells.has(cell)
	):
		for offset in orthogonal:
			if boarding_chokepoint_cells.has(cell + offset):
				bonus += 1
				break
	return bonus

func _unit_label(unit: CombatUnit) -> String:
	return "Player crew" if unit.team == CombatTurnManager.Team.PLAYER else "Enemy crew"

func _team_label(team: CombatTurnManager.Team) -> String:
	return "player" if team == CombatTurnManager.Team.PLAYER else "enemy"
...
func _naval_heading_vector(heading_idx: int) -> Vector2:
	var bearing: float = ship_battle.heading_idx_to_bearing(heading_idx)
	return Vector2.RIGHT.rotated(deg_to_rad(90.0 - bearing)).normalized()

func _naval_cell_center_screen(
	grid_rect: Rect2, cell: Vector2i, view_origin: Vector2, view_cols: int, view_rows: int
) -> Vector2:
...
func _ship_combat_camera_anchor_cells() -> Array[Vector2i]:
	...
	if ship_battle.phase == LegacyShipBattle.Phase.PLANNING and ship_battle.is_player_movement_plotted():
...
func _naval_cell_center_from_float(cell_pos: Vector2, grid_rect: Rect2, view_origin: Vector2, view_cols: int, view_rows: int) -> Vector2:
	...
	var cell_h: float = grid_rect.size.y / float(max(1, view_rows)) * 0.5
	var rx: float = cell_pos.x - view_origin.x
	var ry: float = cell_pos.y - view_origin.y
	
	# Diamond isometric projection:
	# Horizontal axis is X - Y, Vertical axis is X + Y
	var ox: float = grid_rect.position.x + grid_rect.size.x * 0.5
	var oy: float = grid_rect.position.y
	
	var screen_x: float = ox + (rx - ry) * (cell_w * 0.5)
	var screen_y: float = oy + (rx + ry) * (cell_h * 0.5)
	
	return Vector2(screen_x, screen_y)

func _naval_cell_rect(grid_rect: Rect2, cell: Vector2i, view_origin: Vector2, view_cols: int, view_rows: int) -> Rect2:
	var center: Vector2 = _naval_cell_center_from_float(Vector2(cell) + Vector2(0.5, 0.5), grid_rect, view_origin, view_cols, view_rows)
	var cell_w: float = grid_rect.size.x / float(max(1, view_cols))
	var cell_h: float = grid_rect.size.y / float(max(1, view_rows)) * 0.5
	return Rect2(center - Vector2(cell_w, cell_h) * 0.5, Vector2(cell_w, cell_h))

func _combat_to_screen(
	point: Vector2, combat_rect: Rect2, cols: int, rows: int, view_origin: Vector2, view_cols: int, view_rows: int
) -> Vector2:
	if cols <= 0 or rows <= 0:
...
func _naval_combat_view_dims() -> Vector2i:
	return Vector2i(ship_battle.combat_cols, ship_battle.combat_rows)

func _draw_naval_battlefield_grid(
	grid_rect: Rect2,
	view_origin: Vector2,
	view_cols: int,
	view_rows: int
) -> void:
	var combat_cols: int = ship_battle.combat_cols
	var combat_rows: int = ship_battle.combat_rows
	if combat_cols <= 0 or combat_rows <= 0 or view_cols <= 0 or view_rows <= 0:
		return
	var cell_w: float = grid_rect.size.x / float(view_cols)
	var cell_h: float = grid_rect.size.y / float(view_rows)
	var clip: Rect2 = grid_rect.grow(1.0)
	var top: float = grid_rect.position.y
	var bottom: float = grid_rect.position.y + grid_rect.size.y
	var left: float = grid_rect.position.x
	var right: float = grid_rect.position.x + grid_rect.size.x
	var minor_color := Color(0.72, 0.86, 0.95, 0.22)
	var major_color := Color(0.82, 0.94, 1.0, 0.34)
	for x in range(combat_cols + 1):
		var sx: float = grid_rect.position.x + (float(x) - view_origin.x) * cell_w
		if sx < left - 1.0 or sx > right + 1.0:
			continue
		var major: bool = x % 4 == 0
		draw_line(
			Vector2(sx, top),
			Vector2(sx, bottom),
	...
	for y in range(combat_rows + 1):
		var sy: float = grid_rect.position.y + (float(y) - view_origin.y) * cell_h
		if sy < top - 1.0 or sy > bottom + 1.0:
			continue
		var major: bool = y % 4 == 0
		draw_line(
			Vector2(left, sy),
			Vector2(right, sy),
			major_color if major else minor_color,
	...
	var origin_rect: Rect2 = _naval_cell_rect(grid_rect, Vector2i.ZERO, view_origin, view_cols, view_rows)
	var battlefield_rect := Rect2(
		origin_rect.position,
		Vector2(float(combat_cols) * cell_w, float(combat_rows) * cell_h)
	)
	if battlefield_rect.intersects(clip):
		draw_rect(battlefield_rect, Color(0.9, 0.97, 1.0, 0.4), false, 2.0)
...
func _ship_action_button_rect(panel_rect: Rect2, action_id: int) -> Rect2:
	var idx: int = 0
	if action_id == SHIP_ACTION_FIRE_CANNONS:
		idx = 0
	elif action_id == SHIP_ACTION_BOARD:
		idx = 1
	elif action_id == SHIP_ACTION_DISENGAGE:
		idx = 2
...
func _ship_fire_battery_button_rect(panel_rect: Rect2, port: bool) -> Rect2:
	var gap: float = 4.0
	var row_w: float = panel_rect.size.x - 24.0
	var half_w: float = (row_w - gap) * 0.5
	var row_pos: Vector2 = panel_rect.position + Vector2(12.0, SHIP_COMBAT_RANGE_ROW_Y)
	if port:
		return Rect2(row_pos, Vector2(half_w, 36.0))
	return Rect2(row_pos + Vector2(half_w + gap, 0.0), Vector2(half_w, 36.0))
...
func _boarding_end_turn_button_rect(panel_rect: Rect2) -> Rect2:
	return Rect2(
		panel_rect.position + Vector2(12.0, panel_rect.size.y - 108.0),
		Vector2(panel_rect.size.x - 24.0, 36.0)
	)
...
func _draw_boarding_units_overlay(ci: CanvasItem) -> void:
	...
	for unit in units:
		...
		ci.draw_circle(center, radius, base_color)
		ci.draw_string(ThemeDB.fallback_font, center + Vector2(-6.0, 6.0), str(unit.hp), HORIZONTAL_ALIGNMENT_LEFT, -1, 14)
		if unit == selected_unit:
			ci.draw_arc(center, radius + 4.0, 0.0, TAU, 32, Color(1.0, 1.0, 0.5), 3.0)
		if unit == actor:
			ci.draw_arc(center, radius + 8.0, 0.0, TAU, 40, Color(1.0, 0.92, 0.45, 0.95), 3.0)
		if unit.has_acted:
			ci.draw_circle(center, radius, Color(0.0, 0.0, 0.0, 0.4))
		if unit.is_dimmed:
			ci.draw_circle(center, radius + 1.0, Color(0.02, 0.03, 0.06, 0.55))

func _sidebar_compass_invalidate_key() -> String:
	var wind: Dictionary = _get_sidebar_compass_wind()
	var month_key: String = ""
	if game_flow.current_mode == GameLogicState.Mode.WORLD_MAP:
		month_key = ":%d-%02d" % [world_map.game_year, world_map.game_month]
	return "%d%s:%.2f:%.2f" % [
		int(game_flow.current_mode),
		month_key,
		float(wind.get("direction_deg", 90.0)),
		float(wind.get("speed_m_s", 6.0))
	]

func _get_sidebar_compass_wind() -> Dictionary:
	match game_flow.current_mode:
		GameLogicState.Mode.SHIP_COMBAT:
			return {"direction_deg": ship_battle.wind_direction_deg, "speed_m_s": ship_battle.wind_speed_m_s}
		GameLogicState.Mode.WORLD_MAP:
			return world_map.get_current_wind()
		_:
			return {"direction_deg": 90.0, "speed_m_s": 6.0}


func _paint_sidebar_wind_compass(target: CanvasItem) -> void:
	var c: Control = target as Control
	if c == null:
		return
	var wind: Dictionary = _get_sidebar_compass_wind()
	var w: float = maxf(1.0, c.size.x)
	var h: float = maxf(1.0, c.size.y)
	var radius: float = clampf(minf(w * 0.22, h * 0.36), 18.0, 28.0)
	var center := Vector2(w * 0.5, radius + 18.0)
	_draw_wind_compass_on(
		c,
		center,
		float(wind.get("direction_deg", 90.0)),
		float(wind.get("speed_m_s", 6.0)),
...
func _draw_wind_compass_on(
	ci: CanvasItem,
	center: Vector2,
	wind_from_deg: float,
	wind_speed_m_s: float,
	radius: float = 28.0,
	wind_label_font_px: int = 13
) -> void:
	ci.draw_circle(center, radius, Color(0.05, 0.1, 0.14, 0.75))
	ci.draw_arc(center, radius, 0.0, TAU, 48, Color(0.8, 0.9, 1.0, 0.5), 2.0)
	ci.draw_line(center + Vector2(0.0, -radius), center + Vector2(0.0, radius), Color(0.6, 0.75, 0.9, 0.35), 1.0)
	ci.draw_line(center + Vector2(-radius, 0.0), center + Vector2(radius, 0.0), Color(0.6, 0.75, 0.9, 0.35), 1.0)
	ci.draw_string(ThemeDB.fallback_font, center + Vector2(-5.0, -radius - 6.0), "N", HORIZONTAL_ALIGNMENT_LEFT, -1, 12)
	ci.draw_string(ThemeDB.fallback_font, center + Vector2(-4.0, radius + 14.0), "S", HORIZONTAL_ALIGNMENT_LEFT, -1, 12)
	ci.draw_string(ThemeDB.fallback_font, center + Vector2(radius + 6.0, 4.0), "E", HORIZONTAL_ALIGNMENT_LEFT, -1, 12)
	ci.draw_string(ThemeDB.fallback_font, center + Vector2(-radius - 14.0, 4.0), "W", HORIZONTAL_ALIGNMENT_LEFT, -1, 12)
	...
	ci.draw_line(tail, tip, Color(0.44, 0.95, 0.78, 0.95), 2.0)
	...
	ci.draw_colored_polygon(PackedVector2Array([tip, arrow_left, arrow_right]), Color(0.44, 0.95, 0.78, 0.95))
	ci.draw_string(
		ThemeDB.fallback_font,
		center + Vector2(-26.0, radius + 28.0),
		"Wind %.1f m/s" % wind_speed_m_s,
...
func _sync_ship_move_selection() -> void:
	if ship_battle.phase != LegacyShipBattle.Phase.PLANNING:
		return
	var options: Array[Dictionary] = ship_battle.get_player_move_options()
	if options.is_empty():
		ship_move_selected_cell = Vector2i(-1, -1)
		ship_move_selected_heading_idx = -1
		return
	if ship_battle.can_player_move_to_option(ship_move_selected_cell, ship_move_selected_heading_idx):
		return
	var first: Dictionary = options[0]
	ship_move_selected_cell = first.get("cell", Vector2i(-1, -1))
	ship_move_selected_heading_idx = int(first.get("heading_idx", -1))
...
func _move_ship_selection(direction: Vector2i) -> void:
	...
	if ship_battle.phase != LegacyShipBattle.Phase.PLANNING:
		return
	_sync_ship_move_selection()
	if ship_move_selected_cell.x < 0 or ship_move_selected_heading_idx < 0:
		return
	var dir_vec := Vector2(float(direction.x), float(direction.y)).normalized()
	var best_cell: Vector2i = ship_move_selected_cell
	var best_heading: int = ship_move_selected_heading_idx
	var best_score: float = INF
	for opt in ship_battle.get_player_move_options():
		if not (opt is Dictionary):
			continue
		var cell: Vector2i = opt.get("cell", Vector2i(-1, -1))
		var heading_idx: int = int(opt.get("heading_idx", -1))
		if cell.x < 0 or heading_idx < 0:
			continue
		if cell == ship_move_selected_cell and heading_idx == ship_move_selected_heading_idx:
			continue
		var delta: Vector2 = Vector2(
			float(cell.x - ship_move_selected_cell.x),
			float(cell.y - ship_move_selected_cell.y)
		)
		var dot: float = delta.dot(dir_vec)
		if dot <= 0.0:
			continue
		...
		if score < best_score:
			best_score = score
			best_cell = cell
			best_heading = heading_idx
	ship_move_selected_cell = best_cell
	ship_move_selected_heading_idx = best_heading

func _pick_move_option_from_click(clicked_cell: Vector2i) -> Dictionary:
	var options: Array[Dictionary] = ship_battle.get_player_move_options_for_cell(clicked_cell)
	if options.is_empty():
		return {}
	return options[0]
...
func _confirm_ship_move_selection() -> void:
	...
	if ship_move_selected_cell.x < 0 or ship_move_selected_heading_idx < 0:
		return
	if ship_battle.execute_player_move_to_cell(ship_move_selected_cell, ship_move_selected_heading_idx):
		var remaining: int = int(ship_battle.player_plan_terminal.get("mp", ship_battle.player_move_points))
		var face_label: String = SHIP_HEADING_LABELS[posmod(ship_move_selected_heading_idx, 8)]
		game_flow.post_message(
			"Leg plotted to (%d,%d) facing %s — %d MP left after. Enter to move, then fire or plot the next leg."
			% [ship_move_selected_cell.x, ship_move_selected_cell.y, face_label, remaining]
		)
		_sync_ship_move_selection()
	else:
		game_flow.post_message("That move option is not reachable this turn.")
...
func _handle_ship_combat_click(screen_pos: Vector2) -> void:
	...
	var nav_dims: Vector2i = _naval_combat_view_dims()
	var nav_vc: int = nav_dims.x
	var nav_vr: int = nav_dims.y
	var base_grid_rect: Rect2 = _naval_grid_rect(grid_arena_rect, nav_vc, nav_vr)
	...
	if options_rect.has_point(screen_pos):
		for si in range(3):
			if _ship_sail_button_rect(options_rect, si).has_point(screen_pos):
				...
				_sync_ship_move_selection()
				return
		if _ship_fire_battery_button_rect(options_rect, true).has_point(screen_pos):
			ship_combat_range_highlight = 0 if ship_combat_range_highlight != 0 else -1
		elif _ship_fire_battery_button_rect(options_rect, false).has_point(screen_pos):
			ship_combat_range_highlight = 1 if ship_combat_range_highlight != 1 else -1
		elif _ship_action_button_rect(options_rect, SHIP_ACTION_FIRE_CANNONS).has_point(screen_pos):
			if ship_combat_range_highlight >= 0:
				ship_battle.player_fire_battery(ship_combat_range_highlight)
			else:
				game_flow.post_message("Highlight Range Port or Starboard first.")
		elif _ship_action_button_rect(options_rect, SHIP_ACTION_BOARD).has_point(screen_pos):
			ship_battle.player_attempt_boarding()
		elif _ship_action_button_rect(options_rect, SHIP_ACTION_DISENGAGE).has_point(screen_pos):
			if ship_battle.can_player_disengage():
				ship_battle.player_disengage()
			else:
	...
	if not grid_rect.has_point(screen_pos):
		return
	var cell_w: float = grid_rect.size.x / float(nav_vc)
	var cell_h: float = grid_rect.size.y / float(nav_vr) * 0.5
	
	var lx: float = screen_pos.x - (grid_rect.position.x + grid_rect.size.x * 0.5)
	var ly: float = screen_pos.y - (grid_rect.position.y + grid_rect.size.y * 0.25)
	
	# Reverse diamond projection:
	# lx = (rx - ry) * (cell_w * 0.5)
	# ly = (rx + ry) * (cell_h * 0.5)
	# rx - ry = lx / (cell_w * 0.5) => A
	# rx + ry = ly / (cell_h * 0.5) => B
	# 2*rx = A + B => rx = (A + B) / 2
	# 2*ry = B - A => ry = (B - A) / 2
	var a: float = lx / (cell_w * 0.5)
	var b: float = ly / (cell_h * 0.5)
	var rx: float = (a + b) * 0.5
	var ry: float = (b - a) * 0.5
	
	var col_f: float = nav_view_origin.x + rx
	var row_f: float = nav_view_origin.y + ry
	...
	var picked: Dictionary = _pick_move_option_from_click(clicked)
	if not picked.is_empty():
		ship_move_selected_cell = picked.get("cell", Vector2i(-1, -1))
		ship_move_selected_heading_idx = int(picked.get("heading_idx", -1))
		_confirm_ship_move_selection()
...
func _setup_left_sidebar_ui() -> void:
	...
	sidebar_ship_status_label = Label.new()
	sidebar_ship_status_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	sidebar_ship_status_label.add_theme_font_size_override("font_size", 13)
	left_sidebar_panel.add_child(sidebar_ship_status_label)

	sidebar_wind_compass = SidebarWindCompassControl.new()
	sidebar_wind_compass.mouse_filter = Control.MOUSE_FILTER_IGNORE
	sidebar_wind_compass.host = self
	left_sidebar_panel.add_child(sidebar_wind_compass)
...
func _open_main_map_info(title: String, body: String, close_label: String = "Close", return_to_main_menu_on_close: bool = false) -> void:
	...
	main_map_info_layer.visible = game_flow.current_mode == GameLogicState.Mode.WORLD_MAP
	_layout_worldmap_ui()
...
func _setup_port_menu_ui() -> void:
	...
	port_menu_overlay = ColorRect.new()
	port_menu_overlay.color = Color(0.0, 0.0, 0.0, 0.62)
	port_menu_overlay.mouse_filter = Control.MOUSE_FILTER_STOP
	port_menu_layer.add_child(port_menu_overlay)
	...
	port_trade_repair_label = Label.new()
	port_trade_repair_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	port_trade_panel.add_child(port_trade_repair_label)
	var repair_btn_row := HBoxContainer.new()
	repair_btn_row.add_theme_constant_override("separation", 8)
	port_trade_panel.add_child(repair_btn_row)
	port_trade_repair_button = Button.new()
	port_trade_repair_button.text = "Repair Hull"
	port_trade_repair_button.custom_minimum_size = Vector2(180.0, 34.0)
	port_trade_repair_button.focus_mode = Control.FOCUS_CLICK
	port_trade_repair_button.pressed.connect(_on_port_trade_repair_pressed)
	repair_btn_row.add_child(port_trade_repair_button)
...
func _open_port_arrival_menu(port_name: String) -> void:
	...
	port_menu_layer.visible = game_flow.current_mode == GameLogicState.Mode.WORLD_MAP
	_layout_worldmap_ui()
...
func _open_port_docked_menu(port_name: String) -> void:
	...
	var hull_missing: int = _hull_repair_missing_points()
	if hull_missing > 0:
		port_menu_quaternary_button.text = "Repair Hull (%d)" % _hull_repair_cost()
		port_menu_quinary_button.text = "Attack the Port (Land Combat)"
	else:
...
func _on_port_menu_quaternary_pressed() -> void:
	...
	if not port_menu_docked:
		...
		return
	if _hull_repair_missing_points() > 0:
		_execute_hull_repair()
		return
	var target_port := current_port_menu_port
	_close_port_menu()
	last_arrived_port = target_port
...
func _open_port_trade_menu(port_name: String) -> void:
	...
	_refresh_port_repair_offer()

func _open_port_hire_menu(port_name: String) -> void:
	port_menu_screen = "hire"
	port_menu_title_label.text = "%s - Hiring Board" % port_name
	port_menu_info_label.text = "Set how many pirates you want to recruit. Current roster: %d." % crew_roster_size
	port_menu_primary_button.visible = false
	port_menu_secondary_button.visible = false
	port_menu_tertiary_button.visible = false
	port_menu_quaternary_button.visible = false
	...
	port_hire_slider.max_value = float(maxi(0, _crew_roster_cap() - crew_roster_size))
...
func _player_ship_hull_max() -> int:
	return ship_battle.get_ship_max_hull(world_map.ship_class)

func _hull_repair_missing_points() -> int:
	var hull_max: int = _player_ship_hull_max()
	var hull_cur: int = _player_ship_hull_for_battle(world_map.ship_class)
	return maxi(0, hull_max - hull_cur)

func _hull_repair_cost() -> int:
	return _hull_repair_missing_points() * HULL_REPAIR_COST_PER_POINT

func _refresh_port_repair_offer() -> void:
	if port_trade_repair_label == null or port_trade_repair_button == null:
		return
	var missing: int = _hull_repair_missing_points()
	if missing <= 0:
		port_trade_repair_label.text = "Hull is sound (%d/%d). No repairs needed." % [
			_player_ship_hull_for_battle(world_map.ship_class),
			_player_ship_hull_max()
		]
		port_trade_repair_button.disabled = true
		return
	var cost: int = _hull_repair_cost()
	port_trade_repair_label.text = "Hull damage: %d/%d. Full repair costs %d pieces of eight." % [
		_player_ship_hull_for_battle(world_map.ship_class),
		_player_ship_hull_max(),
		cost
	]
	port_trade_repair_button.disabled = cost > player_piasters

func _execute_hull_repair() -> bool:
	var missing: int = _hull_repair_missing_points()
	if missing <= 0:
		game_flow.post_message("Your hull is already fully repaired.")
		_refresh_port_repair_offer()
		_refresh_sidebar_ship_status()
		return false
	var cost: int = _hull_repair_cost()
	if cost > player_piasters:
		game_flow.post_message("Not enough coin for repairs (%d needed)." % cost)
		_refresh_port_repair_offer()
		return false
	player_piasters -= cost
	player_ship_hull_current = _player_ship_hull_max()
	game_flow.post_message(
		"At %s: shipwrights patch %d hull points for %d pieces of eight." % [current_port_menu_port, missing, cost]
	)
	_refresh_sidebar_ship_status()
	if port_menu_active and port_menu_docked:
		_return_to_port_docked_actions()
		port_menu_status_label.text = _port_docked_status_text("Hull restored to fighting trim.")
	elif port_menu_active and port_menu_screen == "trade":
		_refresh_port_repair_offer()
	return true

func _on_port_trade_repair_pressed() -> void:
...
func _on_port_hire_slider_changed(_value: float) -> void:
	...
	var cap_room: int = maxi(0, _crew_roster_cap() - crew_roster_size)
	port_hire_confirm_button.disabled = recruits <= 0 or total_cost > player_piasters or recruits > cap_room
	if recruits > cap_room:
		port_hire_cost_label.text = "Roster cap %d (room for %d)." % [_crew_roster_cap(), cap_room]
...
func _on_port_hire_confirm_pressed() -> void:
	...
	var cap_room: int = maxi(0, _crew_roster_cap() - crew_roster_size)
	if recruits > cap_room:
		port_menu_status_label.text = _port_docked_status_text("Roster is full (%d/%d)." % [crew_roster_size, _crew_roster_cap()])
		return
	player_piasters -= total_cost
	crew_roster_size = mini(_crew_roster_cap(), crew_roster_size + recruits)
	_refresh_sidebar_ship_status()
...
func _port_docked_status_text(lead: String) -> String:
	...
	var hull_max: int = ship_battle.get_ship_max_hull(world_map.ship_class)
	var hull_cur: int = _player_ship_hull_for_battle(world_map.ship_class)
	return "%s\nRank: %s | Fame: %d\nCoin: %d | Supplies: %d | Cargo: %d | Crew: %d | Hull: %d/%d | Governor Favor: %d | Task: %s\nManifest: %s" % [
		lead,
...
func _setup_main_menu_ui() -> void:
	...
	main_menu_overlay.color = Color(0.01, 0.02, 0.04, 0.96)
	main_menu_layer.add_child(main_menu_overlay)
...
func _on_main_menu_dev_ship_combat_pressed() -> void:
	_begin_gameplay_from_main_menu()
	_set_mode(GameLogicState.Mode.SHIP_COMBAT_V2)
	game_flow.post_message("Dev test: ship combat V2 launched.")

func _on_ship_combat_v2_finished(player_won: bool) -> void:
	_set_mode(GameLogicState.Mode.WORLD_MAP)
	if player_won:
		game_flow.post_message("Naval battle won! You sailed away victorious.")
	else:
		game_flow.post_message("Your ship was lost in battle...")
...
func _on_map_retire_pressed() -> void:
	...
	var total := int(CareerSystem.retirement_score(career_state, player_piasters)) + treasure_bonus + favor_bonus
...
func _captain_page_text() -> String:
	...
	var retirement := int(CareerSystem.retirement_score(career_state, player_piasters))
...
func _begin_gameplay_from_main_menu() -> void:
	in_main_menu = false
	if main_menu_layer: main_menu_layer.visible = false
...
func _award_career_fame(amount: int, reason: String) -> void:
	...
	var new_rank: String = str(CareerSystem.rank_for_fame(new_fame))
...
func _player_ship_hull_for_battle(ship_class: String) -> int:
	var hull_max: int = ship_battle.get_ship_max_hull(ship_class)
	if player_ship_hull_current <= 0:
		player_ship_hull_current = hull_max
	return clampi(player_ship_hull_current, 1, hull_max)

func _crew_roster_cap() -> int:
	_ensure_career_state_defaults()
	var ship_crew_max: int = ship_battle.get_ship_max_crew(world_map.ship_class)
	var morale: int = int(career_state.get("crew_morale", 70))
	var morale_mult: float = clampf(0.82 + float(morale) / 220.0, 0.82, 1.18)
	return clampi(int(round(float(ship_crew_max) * morale_mult * 1.35)), 12, CREW_ROSTER_HARD_MAX)

func _effective_battle_crew_count(ship_class: String = "") -> int:
	var cls: String = ship_class if ship_class != "" else world_map.ship_class
	var ship_max: int = ship_battle.get_ship_max_crew(cls)
	return clampi(crew_roster_size, 8, ship_max)

func _ship_battle_context(extra: Dictionary = {}) -> Dictionary:
	var ship_class: String = str(extra.get("player_ship_class", world_map.ship_class))
	var context: Dictionary = extra.duplicate(true)
	context["player_crew"] = _effective_battle_crew_count(ship_class)
	context["player_hull"] = _player_ship_hull_for_battle(ship_class)
	_ensure_career_state_defaults()
	context["player_morale"] = int(career_state.get("crew_morale", 70))
	return context

func _sync_career_from_ship_battle() -> void:
	crew_roster_size = maxi(8, ship_battle.player_crew)
	player_ship_hull_current = maxi(0, ship_battle.player_hull)
	_refresh_sidebar_ship_status()

func _refresh_sidebar_ship_status() -> void:
	if sidebar_ship_status_label == null:
		return
	var show_status: bool = (
		not in_main_menu
		and game_flow.current_mode == GameLogicState.Mode.WORLD_MAP
		and left_sidebar_panel != null
		and left_sidebar_panel.visible
	)
	sidebar_ship_status_label.visible = show_status
	if not show_status:
		return
	_ensure_career_state_defaults()
	var hull_max: int = _player_ship_hull_max()
	var hull_cur: int = _player_ship_hull_for_battle(world_map.ship_class)
	var morale: int = int(career_state.get("crew_morale", 70))
	sidebar_ship_status_label.text = "Hull %d/%d  Crew %d/%d  Morale %d" % [
		hull_cur,
		hull_max,
		crew_roster_size,
		_crew_roster_cap(),
...
func _get_boarding_save_state() -> Dictionary:
	var unit_rows: Array = []
	for unit in units:
		if unit == null or not is_instance_valid(unit):
			continue
		unit_rows.append({
			"team": int(unit.team),
			"cell": {"x": unit.cell.x, "y": unit.cell.y},
			"hp": unit.hp,
			"max_hp": unit.max_hp,
			"has_acted": unit.has_acted
		})
	var initiative_indices: Array = []
	for actor in boarding_initiative_order:
		var idx: int = units.find(actor)
		if idx >= 0:
			initiative_indices.append(idx)
	return {
		"current_boarding_is_player_attacking": current_boarding_is_player_attacking,
		"boarding_round": boarding_round,
		"boarding_defender_hold_progress": boarding_defender_hold_progress,
		"boarding_initiative_index": boarding_initiative_index,
		"boarding_initial_player_boarders": boarding_initial_player_boarders,
		"boarding_initial_enemy_boarders": boarding_initial_enemy_boarders,
		"boarding_selected_action": boarding_selected_action,
		"boarding_actor_moved": boarding_actor_moved,
		"boarding_actor_attacked": boarding_actor_attacked,
		"units": unit_rows,
		"initiative_indices": initiative_indices
	}

func _apply_boarding_save_state(state: Dictionary) -> void:
	for child in units_root.get_children():
		child.queue_free()
	units.clear()
	boarding_initiative_order.clear()
	game_flow.set_tactical_type(GameLogicState.TacticalType.BOARDING)
	boarding_active = true
	current_boarding_is_player_attacking = bool(state.get("current_boarding_is_player_attacking", true))
	boarding_round = int(state.get("boarding_round", 1))
	boarding_defender_hold_progress = int(state.get("boarding_defender_hold_progress", 0))
	boarding_initiative_index = int(state.get("boarding_initiative_index", 0))
	boarding_initial_player_boarders = int(state.get("boarding_initial_player_boarders", 0))
	boarding_initial_enemy_boarders = int(state.get("boarding_initial_enemy_boarders", 0))
	boarding_selected_action = int(state.get("boarding_selected_action", 0))
	boarding_actor_moved = bool(state.get("boarding_actor_moved", false))
	boarding_actor_attacked = bool(state.get("boarding_actor_attacked", false))
	_configure_boarding_layout()
	var unit_rows: Variant = state.get("units", [])
	if unit_rows is Array:
		for row_variant in unit_rows:
			if not row_variant is Dictionary:
				continue
			var row: Dictionary = row_variant
			var team: CombatTurnManager.Team = int(row.get("team", CombatTurnManager.Team.PLAYER)) as CombatTurnManager.Team
			var cell_data: Variant = row.get("cell", {})
			...
			var restored: CombatUnit = units[units.size() - 1]
			restored.set_acted(bool(row.get("has_acted", false)))
	var initiative_indices: Variant = state.get("initiative_indices", [])
	if initiative_indices is Array:
		for idx_variant in initiative_indices:
			var idx: int = int(idx_variant)
			if idx >= 0 and idx < units.size():
				boarding_initiative_order.append(units[idx])
	if boarding_initiative_order.is_empty():
		_start_boarding_initiative()
	else:
		boarding_initiative_index = clampi(boarding_initiative_index, 0, maxi(0, boarding_initiative_order.size() - 1))
		_assign_boarding_unit_labels()
		_refresh_boarding_unit_visuals()
		_begin_boarding_actor_turn()
	_layout_tactical_ui()

func _get_town_assault_save_state() -> Dictionary:
	var unit_rows: Array = []
	for unit in units:
		if unit == null or not is_instance_valid(unit):
			continue
	...
	var reachable: Array = []
	for cell in reachable_cells:
		reachable.append({"x": cell.x, "y": cell.y})
	var selected_index: int = -1
	if selected_unit != null:
		selected_index = units.find(selected_unit)
	return {
		"units": unit_rows,
		"turn_manager": turn_manager.get_save_state(),
		"selected_unit_index": selected_index,
		"reachable_cells": reachable,
		"last_arrived_port": last_arrived_port
	}

func _apply_town_assault_save_state(state: Dictionary) -> void:
	for child in units_root.get_children():
		child.queue_free()
	units.clear()
	selected_unit = null
	...
	if unit_rows is Array:
		for row_variant in unit_rows:
			...
			units[units.size() - 1].set_acted(bool(row.get("has_acted", false)))
	var tm_state: Variant = state.get("turn_manager", {})
	if tm_state is Dictionary:
		turn_manager.apply_save_state(tm_state as Dictionary)
	if state.has("last_arrived_port"):
		last_arrived_port = str(state.get("last_arrived_port", last_arrived_port))
	var selected_index: int = int(state.get("selected_unit_index", -1))
	if selected_index >= 0 and selected_index < units.size():
		_select_unit(units[selected_index])
	var reachable_data: Variant = state.get("reachable_cells", [])
	if reachable_data is Array:
		for cell_variant in reachable_data:
			if cell_variant is Dictionary:
				var c: Vector2i = Vector2i(int(cell_variant.get("x", 0)), int(cell_variant.get("y", 0)))
				reachable_cells.append(c)
		grid.set_highlighted(reachable_cells)
	_layout_tactical_ui()

func _begin_ship_battle_from_encounter(log_text: String) -> void:
	var context: Dictionary = _ship_battle_context(pending_encounter.duplicate())
	encounter_active = false
	encounter_layer.visible = false
	last_ship_battle_encounter_context = pending_encounter.duplicate()
...
func _on_escape_quit_to_menu_pressed() -> void:
	...
	_close_escape_menu()
	in_main_menu = true
	# Tear down any active combat systems before returning to the menu.
	if ship_combat_v2 != null:
		ship_combat_v2.set_active(false)
	game_flow.set_mode(GameLogicState.Mode.WORLD_MAP)
	if main_menu_layer != null:
		main_menu_layer.visible = true
...
func _save_game() -> bool:
	var data: Dictionary = {
		...
		"player_ship_hull_current": player_ship_hull_current,
		...
		"last_ship_battle_enemy_faction": last_ship_battle_enemy_faction,
		"last_ship_battle_player_aggressor": last_ship_battle_player_aggressor
	}
	if game_flow.current_mode == GameLogicState.Mode.SHIP_COMBAT:
		data["ship_battle"] = ship_battle.get_save_state()
	elif (
		game_flow.current_mode == GameLogicState.Mode.TACTICAL_COMBAT
		and game_flow.tactical_type == GameLogicState.TacticalType.BOARDING
		and boarding_active
	):
		data["ship_battle"] = ship_battle.get_save_state()
		data["boarding_save"] = _get_boarding_save_state()
	elif (
		game_flow.current_mode == GameLogicState.Mode.TACTICAL_COMBAT
		and game_flow.tactical_type == GameLogicState.TacticalType.TOWN_ASSAULT
	):
		data["town_assault_save"] = _get_town_assault_save_state()
	if not last_ship_battle_encounter_context.is_empty():
		data["last_ship_battle_encounter_context"] = last_ship_battle_encounter_context.duplicate()
	if game_flow.current_mode == GameLogicState.Mode.SHIP_COMBAT and not last_ship_battle_encounter_context.is_empty():
		data["ship_battle_encounter"] = last_ship_battle_encounter_context.duplicate()
...
func _load_game() -> bool:
	...
	if data.has("player_ship_hull_current"):
		player_ship_hull_current = int(data["player_ship_hull_current"])
	...
	if data.has("career_state") and data["career_state"] is Dictionary:
		...
		_reset_career_state()
	_ensure_career_state_defaults()
	if data.has("last_ship_battle_enemy_faction"):
		last_ship_battle_enemy_faction = str(data["last_ship_battle_enemy_faction"])
	if data.has("last_ship_battle_player_aggressor"):
		last_ship_battle_player_aggressor = bool(data["last_ship_battle_player_aggressor"])
	if data.has("last_ship_battle_encounter_context") and data["last_ship_battle_encounter_context"] is Dictionary:
		last_ship_battle_encounter_context = (data["last_ship_battle_encounter_context"] as Dictionary).duplicate(true)
	elif data.has("ship_battle_encounter") and data["ship_battle_encounter"] is Dictionary:
		last_ship_battle_encounter_context = (data["ship_battle_encounter"] as Dictionary).duplicate(true)
	if data.has("tactical_type"):
		game_flow.set_tactical_type(int(data["tactical_type"]))
	if data.has("mode"):
		var saved_mode: int = int(data["mode"])
		if saved_mode == GameLogicState.Mode.SHIP_COMBAT and data.has("ship_battle") and data["ship_battle"] is Dictionary:
			ship_battle.apply_save_state(data["ship_battle"] as Dictionary)
			_set_mode(GameLogicState.Mode.SHIP_COMBAT)
			game_flow.post_message("Resumed ship combat from save.")
		elif (
			saved_mode == GameLogicState.Mode.TACTICAL_COMBAT
			and int(data.get("tactical_type", GameLogicState.TacticalType.BOARDING)) == GameLogicState.TacticalType.BOARDING
			and data.has("ship_battle")
			and data.has("boarding_save")
		):
			ship_battle.apply_save_state(data["ship_battle"] as Dictionary)
			_apply_boarding_save_state(data["boarding_save"] as Dictionary)
			_set_mode(GameLogicState.Mode.TACTICAL_COMBAT)
			game_flow.post_message("Resumed boarding action from save.")
		elif (
			saved_mode == GameLogicState.Mode.TACTICAL_COMBAT
			and int(data.get("tactical_type", GameLogicState.TacticalType.TOWN_ASSAULT)) == GameLogicState.TacticalType.TOWN_ASSAULT
			and data.has("town_assault_save")
			and data["town_assault_save"] is Dictionary
		):
			_apply_town_assault_save_state(data["town_assault_save"] as Dictionary)
			_set_mode(GameLogicState.Mode.TACTICAL_COMBAT)
			game_flow.post_message("Resumed town assault from save.")
		elif saved_mode == GameLogicState.Mode.TACTICAL_COMBAT or saved_mode == GameLogicState.Mode.SHIP_COMBAT:
			game_flow.post_message("Combat state could not be restored — returning to the world map.")
			_set_mode(GameLogicState.Mode.WORLD_MAP)
		else:
			_set_mode(GameLogicState.Mode.WORLD_MAP)
	else:
...
func _layout_worldmap_ui() -> void:
	...
	if left_sidebar_menu_box != null and sidebar_wind_compass != null:
		var compass_h: float = SIDEBAR_WIND_COMPASS_MIN_HEIGHT
		var compass_gap: float = SIDEBAR_WIND_COMPASS_BOTTOM_GAP
		var menu_bottom_y: float = map_height - compass_h - compass_gap
		left_sidebar_menu_box.position = Vector2(12.0, 52.0)
		left_sidebar_menu_box.size = Vector2(
			maxf(80.0, sidebar_width - 24.0),
			maxf(80.0, menu_bottom_y - 52.0 - 4.0)
		)
		sidebar_wind_compass.position = Vector2(12.0, menu_bottom_y)
		sidebar_wind_compass.size = Vector2(maxf(80.0, sidebar_width - 24.0), compass_h)
	elif left_sidebar_menu_box != null:
		left_sidebar_menu_box.position = Vector2(12.0, 52.0)
		left_sidebar_menu_box.size = Vector2(maxf(80.0, sidebar_width - 24.0), maxf(80.0, map_height - 64.0))

	if sidebar_ship_status_label != null:
		sidebar_ship_status_label.position = Vector2(14.0, 38.0)
		sidebar_ship_status_label.size = Vector2(maxf(80.0, sidebar_width - 28.0), 36.0)
	_refresh_sidebar_ship_status()
...
func _layout_tactical_ui() -> void:
	...
	var sidebar_width: float = float(metrics["sidebar_width"])
	var map_height: float = float(metrics["map_height"])
	var panel_rect: Rect2 = _boarding_action_panel_rect() if game_flow.tactical_type == GameLogicState.TacticalType.BOARDING else Rect2()
	var left: float = sidebar_width + 14.0
	var top: float = 90.0 if game_flow.tactical_type == GameLogicState.TacticalType.BOARDING else 14.0
...
func _crew_label(unit: CombatUnit) -> String:
	if unit == null:
		return "-"
...
func _boarding_turn_sequence_units() -> Array[CombatUnit]:
	var ordered: Array[CombatUnit] = []
	if boarding_initiative_order.is_empty():
		return ordered
	...
	for step in range(boarding_initiative_order.size()):
		...
		var unit: CombatUnit = boarding_initiative_order[idx]
		if unit == null or not is_instance_valid(unit):
			continue
...
func _boarding_turn_position(unit: CombatUnit) -> int:
